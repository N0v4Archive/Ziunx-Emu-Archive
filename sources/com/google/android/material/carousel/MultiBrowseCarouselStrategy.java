...
public final class MultiBrowseCarouselStrategy extends CarouselStrategy {
    ...
    public static final class Arrangement {
        ...
        private void fit(float f, float f2, float f3, float f4) {
            ...
            if (f9 > 0.0f) {
                ...
                f5 = this.largeSize + (min / this.largeCount);
            } else {
                this.mediumSize += min / this.mediumCount;
                f5 = this.largeSize - (min / this.largeCount);
            }
    ...
    public KeylineState onFirstChildMeasuredWithMargins(Carousel carousel, View view) {
        ...
        float f6 = findLowestCostArrangement.largeSize;
        float f7 = f5 + (max2 * f6);
        float f8 = (f6 / 2.0f) + f7;
        int i3 = findLowestCostArrangement.mediumCount;
        if (i3 > 0) {
            f7 = (findLowestCostArrangement.mediumSize / 2.0f) + f8;
        }
        if (i3 > 0) {
            f8 = (findLowestCostArrangement.mediumSize / 2.0f) + f7;
        }
        float f9 = findLowestCostArrangement.smallCount > 0 ? f8 + (findLowestCostArrangement.smallSize / 2.0f) : f7;
        float containerWidth2 = carousel.getContainerWidth() + f3;
        float childMaskPercentage = CarouselStrategy.getChildMaskPercentage(extraSmallSize, findLowestCostArrangement.largeSize, f);
        float childMaskPercentage2 = CarouselStrategy.getChildMaskPercentage(findLowestCostArrangement.smallSize, findLowestCostArrangement.largeSize, f);
        float childMaskPercentage3 = CarouselStrategy.getChildMaskPercentage(findLowestCostArrangement.mediumSize, findLowestCostArrangement.largeSize, f);
        ...
        if (findLowestCostArrangement.mediumCount > 0) {
            addKeylineRange.addKeyline(f7, childMaskPercentage3, findLowestCostArrangement.mediumSize);
        }
        int i4 = findLowestCostArrangement.smallCount;
        if (i4 > 0) {
            addKeylineRange.addKeylineRange(f9, childMaskPercentage2, findLowestCostArrangement.smallSize, i4);
        }
